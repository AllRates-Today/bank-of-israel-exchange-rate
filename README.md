...
Powered by [AllRatesToday](https://allratestoday.com/central-bank-rates-api/boi/). Get a free API key at [allratestoday.com/register](https://allratestoday.com/register) — 300 requests/month, no credit card.
...
```js
import { getRate, getLatestRates } from 'bank-of-israel-exchange-rate';

// One pair at the official Bank of Israel rate
const pair = await getRate('USD', 'ILS', { apiKey: 'art_live_...' });
console.log(pair.rate, pair.rate_date); // e.g. USD -> ILS on the bank's own date
...
```js
import { getRatesForDate, getHistory } from 'bank-of-israel-exchange-rate';
...
const series = await getHistory(
  { source: 'USD', target: 'ILS', from: '2026-01-01' },
  { apiKey: 'art_live_...' }
);
```
...
If Bank of Israel does not print a pair directly, the API resolves it from the bank's table (inverse, or a cross rate via ILS) and flags it `derived: true` with the `method` — so official and computed values are never confused.
